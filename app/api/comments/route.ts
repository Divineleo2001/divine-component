...
export async function POST(req: Request) {
  try {
    ...
    const { success } = await createComment(validatedData);
...
export async function PUT(req: Request) {
  try {
    ...
    const { success } = await updateComment(validatedParams.id, validatedData);
...
export async function DELETE(req: Request) {
  try {
    ...
    const validatedParams = commentIdSchema.parse({ id });
    const { success } = await deleteComment(validatedParams.id);
