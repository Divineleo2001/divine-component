...
export const insertPatientSchema =
  createInsertSchema(patients).omit(timestamps);
export const insertPatientParams = baseSchema
  .extend({
    age: z.coerce.number(),
  })
  .omit({
...
export const updatePatientParams = baseSchema
  .extend({
    age: z.coerce.number(),
  })
  .omit({
...
export type CompletePatient = Awaited<
  ReturnType<typeof getPatients>
>["patients"][number];
